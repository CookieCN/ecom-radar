...
function formatFutureTime(iso: string | null, t: (k: string, ...a: string[]) => string): string {
  if (!iso) return '—'
  const d = new Date(iso)
  const now = new Date()
  const diffMin = Math.ceil((d.getTime() - now.getTime()) / 60000)
  if (diffMin <= 0) return t('list.overdue')
  if (diffMin < 60) return t('list.inMin', String(diffMin))
  const diffH = Math.ceil(diffMin / 60)
  if (diffH < 24) return t('list.inHour', String(diffH))
  const diffD = Math.ceil(diffH / 24)
  if (diffD < 7) return t('list.inDay', String(diffD))
  return d.toLocaleDateString()
}
...
export function CompetitorList({ onViewDetail, onRefresh }: Props): JSX.Element {
  ...
  const handleRefresh = useCallback(
    async (item: CompetitorListItem) => {
      ...
      try {
        await window.api.captureRun({ input: item.url, marketplace: item.marketplace })
        onRefresh()
      } catch {
        /* ignore */
      } finally {
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <span className="cell-asin" onClick={() => window.open(item.url, '_blank')}>
                      {item.asin}
                    </span>
                  </td>
                  <td className="cell-marketplace">{item.marketplace}</td>
                  <td className="cell-price">
                    {formatCurrency(item.latestPrice, item.latestCurrency)}
                    {item.latestPriceType && (
                      <span className="price-context-label">
                        {item.latestPriceType.replace('_', ' ')}
                      </span>
                    )}
                  </td>
                  ...
                  <td className="cell-time">
                    {item.status === 'active' ? formatFutureTime(item.nextRunAt, t) : '—'}
                  </td>
